...
from countriesapp.serializers import CountriesAppSerializer
...
def countries_list(request):
    if request.method == 'GET':
        ...
        if name is not None:
            countries = countries.filter(name__contains=name)

        countries_serializer = CountriesAppSerializer(countries, many=True)
        return JsonResponse(countries_serializer.data, safe=False)

    elif request.method == 'POST':
        countries_data = JSONParser().parse(request)
        countries_serializer = CountriesAppSerializer(data=countries_data)
...
def countries_detail(request,pk):
    ...
    if request.method == 'GET':
        countries_serializer = CountriesAppSerializer(countries)
        return JsonResponse(countries_serializer.data)

    elif request.method == 'PUT':
        countries_data = JSONParser().parse(request)
        countries_serializer = CountriesAppSerializer(countries,data=countries_data)
        if countries_serializer.is_valid():
            countries_serializer.save()
            return JsonResponse(countries_serializer.data)
        return JsonResponse(countries_serializer.errors,status=status.HTTP_400_BAD_REQUEST)
